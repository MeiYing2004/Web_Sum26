...
import {
  ADMIN_CHECKIN_PREVIEW_QUERY,
  parseCheckInRef,
  type CheckInPreview,
} from '@/lib/admin-checkin';
import { CheckInPreviewModal } from '@/components/admin/CheckInPreviewModal';
...
export default function CheckInCard({ onSuccess }: Props) {
  ...
  async function loadPreview(raw: string) {
    const normalized = parseCheckInRef(raw);
    if (!normalized) {
    ...
    try {
      const data = await gql<{ adminCheckInPreview: CheckInPreview }>(
        ADMIN_CHECKIN_PREVIEW_QUERY,
        { ref: normalized },
        { token }
      );
      setPreview(data.adminCheckInPreview);
    } catch (err) {
      setModalOpen(false);
      toast.error(err instanceof Error ? err.message : 'Không thể tải thông tin vé');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function confirmCheckIn() {
    if (!preview?.bookingCode || !preview.canCheckIn) return;
    ...
    try {
      ...
      if (data.checkIn.success) {
        toast.success(data.checkIn.message || 'Check-in thành công');
        setCode('');
        setModalOpen(false);
        setPreview(null);
        onSuccess?.();
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && loadPreview(code)}
